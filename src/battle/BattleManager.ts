...
export class BattleManager implements BattleModule {
    ...
    private validateBattleSetup(battleSetup: BattleSetup): void {
        ...
        allUnits.forEach((unit) => {
            ...
            if (unit.lifePercentage < 0 || unit.lifePercentage > 1) {
                throw new Error(`Unit ${unit.id} has invalid life percentage: ${unit.lifePercentage}`);
            }

            // Validate stat modifiers if present
            if (unit.statModifiers) {
                if (unit.statModifiers.attMultiplier !== undefined && unit.statModifiers.attMultiplier < 0) {
                    throw new Error(`Unit ${unit.id} has invalid attack multiplier`);
                }
                if (unit.statModifiers.lifMultiplier !== undefined && unit.statModifiers.lifMultiplier < 0) {
        ...
        console.log(`Battle validation passed: ${allUnits.length} units positioned`);
    }
    ...
    private isValidPosition(row: number, col: number): boolean {
        return row >= 0 && row < this.config.BOARD_ROWS &&
    ...
    private convertGameUnitToBattleUnit(unit: Unit): BattleUnit {
        return UnitFactory.createBattleUnitFromUnit(unit, `${unit.team}-${unit.row}-${unit.col}`);
    }
    ...
    private generateBattleResult(): BattleResult {
        const gameStateManager = this.game!.getGameState();
        const board = gameStateManager.gameState.board;
        const alive: { team1: BattleUnit[]; team2: BattleUnit[] } = { team1: [], team2: [] };
        const dead: { team1: BattleUnit[]; team2: BattleUnit[] } = { team1: [], team2: [] };
