...
export class BoardRenderer {
  ...
  private drawGrid(ctx: CanvasRenderingContext2D, gameState: GameState): void {
    ...
    for (let col = 0; col <= this.config.BOARD_COLS; col++) {
      ...
      ctx.lineTo(
        x,
        gameState.boardOffsetY + this.config.BOARD_ROWS * gameState.cellHeight,
      );
      ctx.stroke();
    }
    ...
    for (let row = 0; row <= this.config.BOARD_ROWS; row++) {
      ...
      ctx.lineTo(
        gameState.boardOffsetX + this.config.BOARD_COLS * gameState.cellWidth,
        y,
      );
      ctx.stroke();
    }
  ...
  private drawTargetHighlight(
    ...
    const adjustedFillColor = fillColor.replace(
      /0\.\d+/,
      pulseAlpha.toFixed(2),
    );
