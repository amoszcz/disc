...
export class KnightRenderStrategy implements UnitRenderStrategy {
  ...
  public drawSpecialEffects(
    ...
    if (unit.team === 1 && !unit.hasActed && !unit.isSelected) {
      // Assuming current turn checking is done elsewhere
  ...
  private drawSwordGleam(
    ...
    ctx.save();
    ctx.translate(
      centerX + config.UNIT_RADIUS * 0.7,
      centerY - config.UNIT_RADIUS * 0.7,
    );
