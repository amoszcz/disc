...
export class UIRenderer {
    ...
    public drawUI(ctx: CanvasRenderingContext2D, gameState: GameState, canvasWidth: number, canvasHeight: number): void {
    ...
    private drawMenuScreen(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number): void {
        ...
        ctx.font = '24px Arial';
        ctx.fillStyle = '#666';
        ctx.fillText('Turn-based tactical combat', canvasWidth / 2, canvasHeight / 2 - 50);
        ...
        ctx.font = '16px Arial';
        ctx.fillStyle = '#888';
        ctx.fillText('Each unit has ATT (attack) and LIF (life) stats', canvasWidth / 2, canvasHeight / 2 + 120);
        ctx.fillText('Team 1 (blue) vs Team 2 (red)', canvasWidth / 2, canvasHeight / 2 + 145);
    }

    private drawGameUI(ctx: CanvasRenderingContext2D, gameState: GameState, canvasWidth: number): void {
    ...
    private drawPauseOverlay(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number): void {
    ...
    private drawGameOverScreen(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number): void {
        // Semi-transparent overlay
        ...
        ctx.font = '24px Arial';
        ctx.fillText('Press R to return to menu', canvasWidth / 2, canvasHeight / 2 + 20);
    }
    ...
    private drawTeamLabels(ctx: CanvasRenderingContext2D, gameState: GameState): void {
    ...
    private drawLegend(ctx: CanvasRenderingContext2D, gameState: GameState): void {
        ctx.fillStyle = '#666';
        ctx.font = '16px Arial';
        ctx.textAlign = 'left';
        const legendY = gameState.boardOffsetY + this.config.BOARD_ROWS * gameState.cellHeight + 40;
        ctx.fillText('ATT: Attack Damage', 20, legendY);
        ctx.fillText('LIF: Life Points (Health)', 20, legendY + 25);
    }

    private drawTurnIndicator(ctx: CanvasRenderingContext2D, gameState: GameState, canvasWidth: number): void {
